...
#define TS_WILDCARD	4 /* Enable wildcard pattern matching (* and ?) */
...
{
	const char		*name;
	struct ts_config *	(*init)(const void *, unsigned int, int, int);
						...
						int gfp_mask)
{
	...
	conf = calloc(1, TS_PRIV_ALIGN(sizeof(*conf)) + payload);
	if (conf == NULL)
		return NULL;
...
static inline void free_ts_config(struct ts_config *conf)
{
	free(conf);
}
...
{
	return ((uint8_t *) conf + TS_PRIV_ALIGN(sizeof(struct ts_config)));
}
