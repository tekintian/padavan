...
struct nvram_pair router_defaults[] = {
        ...
        { "url_inv_x", "0" },
        /* URL filter runtime mode: 0=auto (DNS+ipset, recommended),
         *                          1=legacy xt_string scan (compat). */
        { "url_filter_mode", "0" },
        /* Sinkhole IPv4 address for blocked domains. Defaults to 0.0.0.0
         * (blackhole). Any user-set value must be a valid IPv4. */
        { "url_sinkhole_ip", "0.0.0.0" },
        /* Kids-control extensions:
         *   url_filter_whitelist=1   strict mode — only listed domains work,
         *                            everything else sinkholed. Recommended
         *                            for young children / study-only devices.
         *   url_force_dns=1          force all :53 (TCP+UDP) from LAN to be
         *                            NAT-redirected to local dnsmasq, so the
         *                            child cannot bypass by configuring 8.8.8.8
         *                            or 1.1.1.1 on their device.
         *   url_block_vpn=1          drop outbound to common VPN / proxy ports
         *                            (OpenVPN 1194, PPTP 1723, L2TP 1701,
         *                            WireGuard 51820, SOCKS 1080, etc.) so the
         *                            child cannot tunnel past the DNS filter.
         *   url_log_blocked=1        syslog every blocked attempt with prefix
         *                            "URL_BLOCK" — lets parents audit which
         *                            domains/IPs the child tried to reach.
         */
        { "url_filter_whitelist", "0" },
        { "url_force_dns", "0" },
        /* Per-MAC differentiation: parents' MACs bypass dnsmasq sinkhole.
         * 12-hex-digit MAC list, semicolon-separated (e.g.
         * "AABBCCDDEEFF;112233445566"). Empty = all devices filtered. */
        { "url_parent_mac_x", "" },
        { "url_block_vpn", "0" },
        /* Optional custom VPN/proxy port list for url_block_vpn=1.
         * Space-separated integers. Empty = use built-in default
         * (1194 51820 = OpenVPN + WireGuard, the two kids most commonly
         * install). Add 1723 1701 500 4500 1080 3128 8080 8443 8888 for
         * stricter coverage. */
        { "url_block_vpn_ports", "" },
        /* DoH/DoT blocking: sinkhole well-known DoH/DoT server domains
         * (dns.google, cloudflare-dns.com, ...) AND drop outbound TCP/UDP
         * 853 (DoT). Closes the Firefox/Chrome/Android-iOS "Secure DNS"
         * bypass. Default off — only needed for kids-control mode. */
        { "url_block_doh", "0" },
        { "url_log_blocked", "0" },
        { "url_num_x", "0" },
