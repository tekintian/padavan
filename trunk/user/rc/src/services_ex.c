...
start_dns_dhcpd(int is_ap_mode)
{
        ...
        /* URL filter (auto-generated by firewall_ex.c::url_filter_dns_update()).
         * File always exists (even if empty) so include is safe.
         *
         * Coordination with Adbyby: Adbyby historically added its own
         * "conf-dir=/etc/storage/dnsmasq-adbyby.d" by sed-editing this file
         * after start_dns_dhcpd() wrote it. That broke every time dnsmasq
         * was restarted without also restarting adbyby. We now include the
         * adbyby conf-dir directly here when APP_ADBYBY is compiled in, so
         * the Adbyby rules survive any dnsmasq restart. */
        fprintf(fp, "conf-file=%s/url_filter.conf\n", storage_dir);

        /* DoH/DoT blocklist (auto-generated when url_block_doh=1).
         * Always-safe empty file when feature is off. */
        fprintf(fp, "conf-file=%s/doh_block.conf\n", storage_dir);

#if defined(APP_ADBYBY)
        fprintf(fp, "conf-dir=/etc/storage/dnsmasq-adbyby.d\n");
#endif
        fclose(fp);
        if (is_dns_used)
