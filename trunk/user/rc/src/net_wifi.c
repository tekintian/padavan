...
void restart_wifi_rt(int radio_on, int need_reload_conf)
{
	...
	if (need_reload_conf)
	{
		...
		// 修改start_8021x_wl函数（5G radio的802.1x服务）
		void
		start_8021x_wl(void)
		{
#if BOARD_HAS_5G_RADIO
			...
			{
				// 读取调试级别参数，默认为0（不记录日志）
				int debug_level = nvram_get_int("rt_radius_debug_level");
				// 如果启用了调试（级别>0），则传递-d参数
				if (debug_level > 0)
				{
					eval("rt2860apd", "-i", IFNAME_5G_MAIN, "-d", NULL);
				}
				else
				{
		...
		// 修改start_8021x_rt函数（2G radio的802.1x服务）
		void
		start_8021x_rt(void)
		{
#if !defined(USE_RT3352_MII)
			...
			if (is_need_8021x(nvram_wlan_get(0, "auth_mode")))
			{
				...
				if (debug_level > 0)
				{
					eval("rtinicapd", "-i", IFNAME_2G_MAIN, "-d", NULL);
				}
				else
				{
