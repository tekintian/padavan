...
start_8021x_wl(void)
{
    ...
    // 检查调试级别
    int rt_radius_debug_level = nvram_get_int("rt_radius_debug_level");
    const char *auth_mode = nvram_wlan_get(1, "auth_mode");
    
    // 记录调试日志
    if (rt_radius_debug_level > 0) {
        logmessage(LOGNAME, "DEBUG: start_8021x_wl: enabled_radio_wl=%d, auth_mode=%s", 
                  get_enabled_radio_wl(), auth_mode);
    }
    
    if (is_need_8021x(auth_mode)) {
        if (rt_radius_debug_level > 0) {
            // 启用调试模式，添加"-d"参数并记录日志
            logmessage(LOGNAME, "DEBUG: start_8021x_wl: starting rt2860apd in debug mode for interface %s", IFNAME_5G_MAIN);
            eval("rt2860apd", "-d", "-i", IFNAME_5G_MAIN);
        } else {
            // 正常模式
            eval("rt2860apd", "-i", IFNAME_5G_MAIN);
        }
    } else if (rt_radius_debug_level > 0) {
        logmessage(LOGNAME, "DEBUG: start_8021x_wl: 802.1x not needed for auth_mode=%s", auth_mode);
    }

    const char *wifname = find_wlan_if_up(1);
    if (wifname && rt_radius_debug_level > 0) {
        logmessage(LOGNAME, "DEBUG: start_8021x_wl: found interface %s", wifname);
    }
    ...
    if (rt_radius_debug_level > 0) {
        logmessage(LOGNAME, "DEBUG: start_8021x_wl: setting KickStaRssiLow=%d, AssocReqRssiThres=%d on %s", 
                   wl_KickStaRssiLow, wl_AssocReqRssiThres, wifname);
    }
...
start_8021x_rt(void)
{
    ...
    // 检查调试级别
    int rt_radius_debug_level = nvram_get_int("rt_radius_debug_level");
    const char *auth_mode = nvram_wlan_get(0, "auth_mode");
    
    // 记录调试日志
    if (rt_radius_debug_level > 0) {
        logmessage(LOGNAME, "DEBUG: start_8021x_rt: enabled_radio_rt=%d, auth_mode=%s", 
                  get_enabled_radio_rt(), auth_mode);
    }
    
    if (is_need_8021x(auth_mode)) {
        if (rt_radius_debug_level > 0) {
            // 启用调试模式，添加"-d"参数并记录日志
            logmessage(LOGNAME, "DEBUG: start_8021x_rt: starting rtinicapd in debug mode for interface %s", IFNAME_2G_MAIN);
            eval("rtinicapd", "-d", "-i", IFNAME_2G_MAIN);
        } else {
            // 正常模式
            eval("rtinicapd", "-i", IFNAME_2G_MAIN);
        }
    } else if (rt_radius_debug_level > 0) {
        logmessage(LOGNAME, "DEBUG: start_8021x_rt: 802.1x not needed for auth_mode=%s", auth_mode);
    }

    const char *wifname = find_wlan_if_up(0);
    if (wifname && rt_radius_debug_level > 0) {
        logmessage(LOGNAME, "DEBUG: start_8021x_rt: found interface %s", wifname);
    }
    ...
    if (rt_radius_debug_level > 0) {
        logmessage(LOGNAME, "DEBUG: start_8021x_rt: setting KickStaRssiLow=%d, AssocReqRssiThres=%d on %s", 
                   rt_KickStaRssiLow, rt_AssocReqRssiThres, wifname);
    }
