...
{
        local interval=$1
        [ -z "$http_username" ] && http_username="root"
        local cron_file="/etc/storage/cron/crontabs/$http_username"
        [ -f "$cron_file" ] || return 0

        # Atomically rewrite the health_check entry: drop old, append new.
        sed -i '/adbyby\.sh health_check/d' "$cron_file"
        printf '*/%s * * * * /bin/sh /usr/bin/adbyby.sh health_check >/dev/null 2>&1\n' \
                "$interval" >> "$cron_file"
}
...
adbyby_start()
{
        ...
        hosts_ads
        /sbin/restart_dhcpd
        # add_cron already installs BOTH the rule-update task AND the
        # health-check task atomically. The previous version then appended
        # a SECOND health_check entry here, causing cron to invoke it
        # twice per hour (and piling up across restarts).
        add_cron
...
add_cron()
{
        # Robustness: never operate on an empty $http_username — that would
        # turn the crontabs path into a directory and silently corrupt it.
        [ -z "$http_username" ] && http_username="root"

        local cron_dir="/etc/storage/cron/crontabs"
        mkdir -p "$cron_dir"
        local cron_file="$cron_dir/$http_username"
        [ -f "$cron_file" ] || touch "$cron_file"

        # Remove ALL adbyby-related entries (rule update + health check) so
        # we never accumulate stale duplicates across restarts. The previous
        # version only matched the literal string "adbyby" which left the
        # health_check job behind, piling up across reboots.
        sed -i '/adbyby\.sh/d' "$cron_file"

        if [ "$adbyby_update" = "0" ]; then
                # Daily update at HH:MM
                printf '%s %s * * * /bin/sh /usr/bin/adbyby.sh G >/dev/null 2>&1\n' \
                        "$adbyby_update_min" "$adbyby_update_hour" >> "$cron_file"
                logger -t "adbyby" "设置每天$adbyby_update_hour时$adbyby_update_min分，自动更新规则！"
        elif [ "$adbyby_update" = "1" ]; then
                # Every N minutes (adbyby_update_min) within every M hours (adbyby_update_hour)
                printf '*/%s */%s * * * /bin/sh /usr/bin/adbyby.sh G >/dev/null 2>&1\n' \
                        "$adbyby_update_min" "$adbyby_update_hour" >> "$cron_file"
                logger -t "adbyby" "设置每隔$adbyby_update_hour时$adbyby_update_min分，自动更新规则！"
        fi
        # adbyby_update=2: no rule-update cron at all (manual only)

        # Health-check cron — single source of truth, always added so the
        # service can self-heal. update_cron_interval() will rewrite this
        # line in-place based on failure count.
        if ! grep -q 'adbyby\.sh health_check' "$cron_file"; then
                printf '*/60 * * * * /bin/sh /usr/bin/adbyby.sh health_check >/dev/null 2>&1\n' \
                        >> "$cron_file"
                logger -t "adbyby" "已添加adbyby智能健康检查任务（默认1小时）"
        fi
}
...
del_cron()
{
        ...
        local cron_file="/etc/storage/cron/crontabs/$http_username"
        [ -f "$cron_file" ] || return 0
        # Remove every adbyby-related line (rule update + health check).
        sed -i '/adbyby\.sh/d' "$cron_file"
}
...
ip_rule()
{
        ...
        # Use -exist so repeated starts don't fail with "set already exists".
        # The previous code used bare `ipset -N` which errored out every time
        # adbyby was restarted without first being fully torn down.
        ipset -N adbyby_esc hash:ip -exist 2>/dev/null || ipset create adbyby_esc hash:ip -exist 2>/dev/null
                ...
                if [ $num -ne 0 ]; then
                        ...
                        [ -z "$mode" ] && mode=0
                                ...
                                1)
                                        ...
                                        ;;
                                2)
                                        ipset -N adbyby_wan hash:ip -exist 2>/dev/null || ipset create adbyby_wan hash:ip -exist 2>/dev/null
        ...
        # blockip set: hash:net, also -exist for idempotent restarts.
        ipset create blockip hash:net family inet hashsize 1024 maxelem 65536 -exist 2>/dev/null
        if [ -f "$adbyby_dir/blockip.conf" ]; then
                awk '!/^$/&&!/^#/{printf("add blockip %s '" "'\n",$0)}' $adbyby_dir/blockip.conf > /tmp/blockip.ipset
                ipset -! restore < /tmp/blockip.ipset 2>/dev/null
        fi
        # Only insert the FORWARD/OUTPUT jumps if they're not already there
        # (idempotent — previously every restart added a new chain jump).
        iptables -C FORWARD -m set --match-set blockip dst -j DROP 2>/dev/null || \
                iptables -I FORWARD -m set --match-set blockip dst -j DROP
        iptables -C OUTPUT -m set --match-set blockip dst -j DROP 2>/dev/null || \
...
del_dns()
{
        # Only remove the adbyby-specific conf-dir line that we own. The
        # previous version did `sed -i '/hosts/d'` which nuked every line
        # containing "hosts" — including `addn-hosts=`, `dhcp-hostsfile=`,
        # breaking static DHCP assignments and /etc/hosts injection.
        sed -i '/dnsmasq-adbyby/d' /etc/storage/dnsmasq/dnsmasq.conf
...
add_rule()
{
        # Idempotent chain creation: `-N` fails if the chain already exists,
        # which previously caused add_rule() to abort mid-way on restart.
        # Create the chain only when missing.
        if ! $ipt_n -n -L ADBYBY >/dev/null 2>&1; then
                $ipt_n -N ADBYBY
        fi

        # Reset the chain to a known-empty state so re-applying the rules
        # below never accumulates duplicates.
        ...
        # Insert PREROUTING jump only if not already present (idempotent).
        if ! $ipt_n -C PREROUTING -p tcp --dport 80 -j ADBYBY 2>/dev/null; then
