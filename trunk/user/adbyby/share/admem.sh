...
if [ ! -f "/tmp/adbyby.updated" ];then
  ...
  while [ "$wget_ok" = "0" ] 
    ...
    if [ "$?" == "0" ]; then
      wget_ok="1"
      touch /tmp/lazy.txt && wget-ssl --no-check-certificate -t 1 -T 10 -O /tmp/lazy.txt https://gitee.com/tekintian/adt-rules/raw/master/adbyby/lazy.txt
      touch /tmp/video.txt && wget-ssl --no-check-certificate -t 1 -T 10 -O /tmp/video.txt https://gitee.com/tekintian/adt-rules/raw/master/adbyby/video.txt
      
      touch /tmp/local-md5.json && md5sum /tmp/lazy.txt /tmp/video.txt > /tmp/local-md5.json
      touch /tmp/md5.json && wget-ssl --no-check-certificate -t 1 -T 10 -O /tmp/md5.json https://gitee.com/tekintian/adt-rules/raw/master/adbyby/md5.json
